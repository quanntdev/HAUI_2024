  ...
  async (dispatch: any) => {
    try {
      ...
      const apiResponse: any = await baseAxios.publicRequest({
        url: process.env.NEXT_PUBLIC_API_ADDRESS + `/cities/${querySearch}`,
        data: { querySearch },
...
export const createCities = (body: any) => async (dispatch: any) => {
  try {
    dispatch({
      type: t.LOADING,
      payload: true,
    ...
    const apiResponse: any = await baseAxios.publicRequest({
      url: process.env.NEXT_PUBLIC_API_ADDRESS + `/cities`,
      data: body,
    ...
    if (
      ...
      dispatch({ type: t.CREATE_CITITES, payload: apiResponse });
    } else if (
      apiResponse?.error ||
